...
train_knn <- train(factor(Potability) ~ ., method = "knn",
                   tuneGrid = data.frame(k = seq(3, 45, 2)), #31
                   data = train_data_1, 
...
train_svm1 <- train(factor(Potability) ~., method = "lssvmRadial",
                    data = train_data_2,
                    tuneGrid = expand.grid(
                      tau = 0.001, #0.001	c(0.001,0.009,0.01,0.1)
                      sigma = 0.009 #0.009	c(0.001,0.009,0.01,0.1)
                    ), 
                    trControl = train_control) 
...
train_xgbT1 <- train(factor(Potability) ~., data = train_data_2, 
                     ...
                     trControl = train_control,
                     tuneGrid = expand.grid(
                       nrounds=70, # c(63,65,70)
                       eta=0.01, #c(0.01, 0.05)
                       max_depth=8,#c(6, 7, 8)
                       colsample_bytree=1, #c(0.5, 1)
                       subsample=0.5, 
                       gamma=0.001, #c(0.0001, 0.001, 0.01)
                       min_child_weight=1)) #c(0,1,2)
